...
def kafka_prod():
    ...
    for data in data_list:
        producer.send('benefit_12', data.encode('utf-8'))
    producer.flush()
...
def spark_kafka():
    ...
    output_in_query = kafka_in_value_df.writeStream \
                          .queryName("benef432322") \
                          .format("memory") \
                          .start()
    output_in_query.awaitTermination(10)
       
    in_value_df = spark.sql("select * from benef432322")
...
def main():
     ...
     spark_kafka()
main()   
print('Benefit part 3 data is successfully loaded to MongoDB')  